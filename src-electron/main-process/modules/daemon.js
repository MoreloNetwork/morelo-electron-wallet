...
export class Daemon {
    ...
    checkVersion() {
        return new Promise((resolve, reject) => {
            if (process.platform === "win32") {
                let morelod_path = path.join(__morelo_bin, "morelod.exe")
                let morelod_version_cmd = `"${morelod_path}" --version`
                if (!fs.existsSync(morelod_path))
                    resolve(false)
                child_process.exec(morelod_version_cmd, (error, stdout, stderr) => {
                    if(error)
                        resolve(false)
                    resolve(stdout)
                })
            } else {
                let morelod_path = path.join(__morelo_bin, "morelod")
                let morelod_version_cmd = `"${morelod_path}" --version`
                if (!fs.existsSync(morelod_path))
                    resolve(false)
                child_process.exec(morelod_version_cmd, {detached: true}, (error, stdout, stderr) => {
                    if(error)
                        resolve(false)
                    resolve(stdout)
                })
     ...
     checkRemoteHeight() {
        let url = "http://explorer.morelo-network.com/api/networkinfo"
        if(this.testnet) {
    ...
    start(options) {
        ...
        return new Promise((resolve, reject) => {
            ...
            if (process.platform === "win32") {
                this.daemonProcess = child_process.spawn(path.join(__morelo_bin, "morelod.exe"), args)
            } else {
                this.daemonProcess = child_process.spawn(path.join(__morelo_bin, "morelod"), args, {
                    detached: true
                })
            }
