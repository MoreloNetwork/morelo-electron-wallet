        ...
        return new Promise((resolve, reject) => {
            ...
            crypto.randomBytes(64+64+32, (err, buffer) => {
                ...
                if (process.platform === "win32") {
                    this.walletRPCProcess = child_process.spawn(path.join(__morelo_bin, "morelo-wallet-rpc.exe"), args)
                } else {
                    this.walletRPCProcess = child_process.spawn(path.join(__morelo_bin, "morelo-wallet-rpc"), args, {
                        detached: true
                    })
                }
...
console.log('success')
    ...
    listWallets(legacy=false) {
        ...
        if(legacy) {
	        ...
	        legacy_paths = ["C:\\ProgramData\\morelo"]
            } else {
	        legacy_paths = [path.join(os.homedir(), "Morelo")]
            }
